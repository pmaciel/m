...
preplot that can process IEEE binary files on a CRAY.  The procedure to
follow is:
...
#define TecplotBinaryFileVersion 111 /* copied from GLOBAL.h */
#define VersionString "PREPLOT V11.2-0  06/05/2007  Copyright (C) Tecplot, Inc. 1987-2008.\n"
...
/*
 * Must define TRUE before including Q_UNICODE.h ....
 */
#define   FALSE             0
#define   TRUE              1
...
typedef enum FileType {FULLFILE,
                       GRIDFILE,
                       SOLUTIONFILE}                    FileType;
/* deprecated */
...
static LgIndex           *IMax;
static LgIndex           *JMax;
static LgIndex           *MaxPolyBndryConnElem;
static LgIndex           *ZoneWithMaxConnElem;
static LgIndex            MaxPolyBndryConnZone = 0;
static LgIndex            ZoneWithMaxBndryConnZone = 0;
static LgIndex           *KMax;
static LgIndex           *TotalNumFaceNodes;
static LgIndex           *TotalNumBndryFaces;
static LgIndex           *TotalNumBndryConns;
...
#             endif
  ...
  if (MaxPolyBndryConnElem)     FREE_ARRAY(MaxPolyBndryConnElem,"MaxPolyBndryConnElem");
  if (ZoneWithMaxConnElem)      FREE_ARRAY(ZoneWithMaxConnElem,"ZoneWithMaxConnElem");
  if (KMax)                     FREE_ARRAY(KMax,"KMax");
  if (TotalNumFaceNodes)        FREE_ARRAY(TotalNumFaceNodes,"TotalNumFaceNodes");
  if (TotalNumBndryFaces)       FREE_ARRAY(TotalNumBndryFaces,"TotalNumBndryFaces");
  if (TotalNumBndryConns)       FREE_ARRAY(TotalNumBndryConns,"TotalNumBndryConns");
  ...
  IMax                  = NULL;
  JMax                  = NULL;
  MaxPolyBndryConnElem  = NULL;
  ZoneWithMaxConnElem   = NULL;
  KMax                  = NULL;
  TotalNumFaceNodes     = NULL;
  TotalNumBndryFaces    = NULL;
  TotalNumBndryConns    = NULL;
  ZoneType              = NULL;
  DataPacking           = NULL;
  CurTSet               = NULL;
  IStart                = NULL;
  ...
  MaxPolyBndryConnZone = 0;
  ZoneWithMaxBndryConnZone = 0;
...
static Boolean_t AllocatePreplotStuff(void)
{
  ...
  MaxPolyBndryConnElem  = ALLOC_ARRAY(MaxNumZones,LgIndex,"MaxPolyBndryConnElem");
  ZoneWithMaxConnElem   = ALLOC_ARRAY(MaxNumZones,LgIndex,"ZoneWithMaxConnElem");
  KMax                  = ALLOC_ARRAY(MaxNumZones,LgIndex,"KMax");
  TotalNumFaceNodes     = ALLOC_ARRAY(MaxNumZones,LgIndex,"TotalNumFaceNodes");
  TotalNumBndryFaces    = ALLOC_ARRAY(MaxNumZones,LgIndex,"TotalNumBndryFaces");
  TotalNumBndryConns    = ALLOC_ARRAY(MaxNumZones,LgIndex,"TotalNumBndryConns");
...
static void ErrorMessage(const char *S,
                         Boolean_t   ShowLine)
{
  REQUIRE(VALID_BOOLEAN(ShowLine));
  PreplotIsOk = FALSE;
  if (ErrProcessed)
    return;
...
static void ErrorMessageZone(const char *S,
                             LgIndex     ZoneNum)
{
  REQUIRE(ZoneNum > 0);
  PreplotIsOk = FALSE;
          ...
          if (!IsProcessingAuxData)
            {
              ...
              else if (CheckTokenStr("FILETYPE"))
                CurToken = FILETYPEToken;
            }
        }
                                  ...
                                  double          *VarMaxValue)
{
  ...
  REQUIRE((VarMinValue == NULL) || (VALID_REF(VarMinValue)));
  REQUIRE((VarMinValue == NULL) || (VALID_REF(VarMaxValue)));
  ...
  if (RemoveEqualsSign(FALSE))
    {
      if (CheckTokenStr("ORDERED"))
        ...
        Result = FEBRICK;
      else if (CheckTokenStr("FEPOLYGON"))
        Result = FEPOLYGON;
      else if (CheckTokenStr("FEPOLYHEDRON"))
        Result = FEPOLYHEDRON;
      else
        ErrorMessage("Invalid ZONETYPE, Need ORDERED, FELINESEG, FETRIANGLE, "
               "FEQUADRILATERAL, FETETRAHEDRON, FEBRICK, FEPOLYGON, or FEPOLYHEDRON",TRUE);
    }
  else
...
/**
 */
static FileType GetFileType(void)
{
  FileType Result = (FileType)-1;

  if (RemoveEqualsSign(FALSE))
    {
      if (CheckTokenStr("FULL"))
        Result = FULLFILE;
      else if (CheckTokenStr("GRID"))
        Result = GRIDFILE;
      else if (CheckTokenStr("SOLUTION"))
        Result = SOLUTIONFILE;
      else
        ErrorMessage("Invalid FILETYPE in file header. Must be FULL, GRID, or SOLUTION", TRUE);
    }
  else
    ErrorMessage ("Missing Equals sign for FILETYPE Specifier",TRUE);
...
static void RecogZoneHeader(void)
{
  ...
  Boolean_t           FaceNodesSet         = FALSE;
  Boolean_t           BndryFacesSet        = FALSE;
  Boolean_t           BndryConnsSet        = FALSE;
  ...
  ICellDim             = 0; /* ...initially not defined */
  JCellDim             = 0; /* ...initially not defined */
  KCellDim             = 0; /* ...initially not defined */
  NodeVar              = -1;
  ...
  while ((CurToken == IdentToken) && (PreplotIsOk))
    {
      ...
      /* Deprecated as of binary version 110: use Nodes */
      ...
      else if (CompareIdent(CurData,"Nodes"))
        {
          GetAnInteger(&IMax[CurZone]);
          FEFlagsUsed = TRUE;
          ISet = TRUE;
      ...
      /* Deprecated as of binary version 110: use Elements */
      ...
      else if (CompareIdent(CurData,"Elements"))
        {
          GetAnInteger(&JMax[CurZone]);
          FEFlagsUsed = TRUE;
          JSet = TRUE;
      ...
      else if (CompareIdent(CurData,"Faces"))
        {
          GetAnInteger(&KMax[CurZone]);
          FEFlagsUsed = TRUE;
          KSet = TRUE;
  ...
  if (PreplotIsOk                  &&
      ZoneType[CurZone] != ORDERED &&
      NVSet && !ISet)
    ErrorMessage("Cannot specify NV without also specifying Nodes or N (deprecated)",TRUE);

  if (PreplotIsOk && FEFlagsUsed && ZoneType[CurZone] == ORDERED)
    ErrorMessage("Nodes, Elements, Faces, N, E, NV, ICELLDIM, JCELLDIM, KCELLDIM "
  ...
  if (PreplotIsOk && Echo)
    {
      ...
      if (ZoneType[CurZone] == FEPOLYGON ||
          ZoneType[CurZone] == FEPOLYHEDRON)
             {
          if (TotalNumFaceNodes[CurZone]  >= 0)
            printf("             TotalNumFaceNodes=  %d\n",TotalNumFaceNodes[CurZone]);
          if (TotalNumBndryFaces[CurZone] >= 0)
            printf("             TotalNumBndryFaces= %d\n",TotalNumBndryFaces[CurZone]);
          if (TotalNumBndryConns[CurZone] >= 0)
            printf("             TotalNumBndryConns= %d\n",TotalNumBndryConns[CurZone]);
        }
  ...
  if (PreplotIsOk && !ISet && ZoneType[CurZone] != ORDERED)
    ErrorMessage("Must Specify Nodes or N (deprecated) (greater than 0) for FE formats",TRUE);
  ...
  if (PreplotIsOk &&
      (IMax[CurZone] != 0 || JMax[CurZone] != 0 || KMax[CurZone] != 0))
    {
      if (ZoneType[CurZone] == ORDERED)
        ...
        {
          if (IMax[CurZone] <= 0 )
            ErrorMessage("Nodes or N (deprecated) must be greater than 0 if specified",TRUE);
          else if ( JMax[CurZone] <= 0 )
            ErrorMessage("Elements or E (deprecated) must be greater than 0 if specified",TRUE);
          else if ( KMax[CurZone] <= 0 )
            ErrorMessage("Faces must be greater than 0 if specified",TRUE);
        }
    }
  ...
  if (PreplotIsOk)
    {
      /*
       * The new ASCII format wants these items specified in the zone header
       * although we do accommodate the old format of specifying them in the
       * data section.
       */
      if ((ZoneType[CurZone] == FEPOLYGON ||
           ZoneType[CurZone] == FEPOLYHEDRON) &&
          (FaceNodesSet || BndryFacesSet || BndryConnsSet))
        {
          /*
           * Polygonal data has a homogeneous number of face nodes so if it is
           * not specified we assign it for the user however we are careful not
           * to do this if none of the items were specified because the items
           * may be in the data section (the old ASCII format).
           */
          if (PreplotIsOk &&
              ZoneType[CurZone] == FEPOLYGON &&
              (!FaceNodesSet && (BndryFacesSet || BndryConnsSet)))
            TotalNumFaceNodes[CurZone] = 2 * KMax[CurZone];

          if (PreplotIsOk &&
              ZoneType[CurZone] == FEPOLYGON &&
              (FaceNodesSet && TotalNumFaceNodes[CurZone] != 2 * KMax[CurZone]))
            ErrorMessage("TotalNumFaceNodes for Polygonal zones must be equal to 2 * the number of faces in the zone.", TRUE);

          if (PreplotIsOk &&
              ZoneType[CurZone] == FEPOLYHEDRON &&
              TotalNumFaceNodes[CurZone] < 1)
            ErrorMessage("Polyhedron zones must specify TotalNumFaceNodes.", TRUE);

          if (PreplotIsOk &&
              ((BndryFacesSet != BndryConnsSet) ||
               (FaceNodesSet && (!BndryFacesSet || !BndryConnsSet))))
            ErrorMessage("NumConnectedBoundaryFaces and TotalNumBoundaryConnections must both be explicitly set for face map data.", TRUE);

          if (PreplotIsOk &&
              (TotalNumBndryFaces[CurZone] == 0 &&
               TotalNumBndryConns[CurZone] > 0)  ||
              (TotalNumBndryFaces[CurZone] > 0  &&
               TotalNumBndryConns[CurZone] == 0))
            ErrorMessageZone("NumConnectedBoundaryFaces and TotalNumBoundaryConnections must both be zero or both be non-zero.", CurZone+1);
        }
    }
}
...
/*
 * Read a number of int values and write them at the same time
 */
Boolean_t ReadWritePolyIntBlock(LgIndex   NumValues,
                                LgIndex   MinAllowed,
                                LgIndex   MaxAllowed,
                                Boolean_t UseMinMax,
                                FILE     *InFile,
                                FILE     *OutFile)
{
  ...
  REQUIRE(NumValues > 0);
  REQUIRE(MinAllowed <= MaxAllowed);
  REQUIRE(VALID_BOOLEAN(UseMinMax));
  REQUIRE(VALID_REF(InFile));
  REQUIRE(VALID_REF(OutFile));

  for (ValueIndex = 0; Result && ValueIndex < NumValues; ValueIndex++)
    {
      if (CurToken != ValueToken)
        {
          Result = FALSE;
          ErrorMessage("Invalid token found: expected an integer.", TRUE);
        }
      else if (Result && UseMinMax)
        {
          if (CurTValue < MinAllowed ||
              CurTValue > MaxAllowed)
            {
              char ErrMsg[256];
...
/**
 */
Boolean_t ReadWritePolyMaps(LgIndex  NumElems,
                            LgIndex  NumFaces,
                            LgIndex  NumNodes,
                            LgIndex *TotalNumFaceNodes,
                            LgIndex *TotalNumBndryFaces,
                            LgIndex *TotalNumBndryConns,
                            FILE    *InFile,
                            FILE    *BlckFile)
{
  LgIndex   DataIndex;
  ...
  REQUIRE(NumFaces > 0);
  REQUIRE(NumNodes > 0);
  REQUIRE((CurToken == ValueToken &&
           *TotalNumFaceNodes  >= 0 &&     // ...values were specified in the zone header
           *TotalNumBndryFaces >= 0 &&
           *TotalNumBndryConns >= 0)   ||
          (CurToken == IdentToken &&
           *TotalNumFaceNodes  == -1 &&    // ...accommodate the old format of specifying these items here in the data section
           *TotalNumBndryFaces == -1 &&
           *TotalNumBndryConns == -1));
  REQUIRE(VALID_REF(InFile));
  REQUIRE(VALID_REF(BlckFile));

  if (CurToken == IdentToken)
    {
      /*
       * The old ASCII format allowed the user to specify these items in the
       * data section. The new format wants them specified in the zone header
       * but we can accommodate the old format because we haven't yet written
       * the header for this zone.
       */
      Boolean_t BndryFacesSet = FALSE;
      Boolean_t BndryConnsSet = FALSE;
      while (PreplotIsOk && (CurToken == IdentToken))
        {
          if (CompareIdent(CurData, "TotalNumFaceNodes"))
            {
              GetAnInteger(TotalNumFaceNodes);
              if (ZoneType[CurZone] == FEPOLYGON &&
            ...
            {
              ErrorMessage("Invalid identifier in face map.", TRUE);
            }
          if (PreplotIsOk)
            GetToken(FALSE,TRUE,InFile);
        }
  ...
  if (PreplotIsOk && (ZoneType[CurZone] == FEPOLYHEDRON))
    {
      /* Read the number of nodes in each face and write the offset array (polyhedron only) */
      LgIndex NodeOffset = 0;

      PreplotIsOk = WriteBinaryInt(BlckFile, NodeOffset);
      for (DataIndex = 0; PreplotIsOk && DataIndex < NumFaces; DataIndex++)
        {
          if (CurToken != ValueToken)
            {
              ErrorMessage("Invalid token found: expected a node count, one for each face.", TRUE);
            }
          NodeOffset += (LgIndex)CurTValue;
          PreplotIsOk = WriteBinaryInt(BlckFile, NodeOffset);
          GetToken(FALSE,TRUE,InFile);
        }
    }
  if (PreplotIsOk)
    PreplotIsOk = (ReadWritePolyIntBlock(*TotalNumFaceNodes, 1, NumNodes, TRUE, InFile, BlckFile));     /* ...face nodes */
  if (PreplotIsOk)   /* indexsize = NumFaces, min = -(*TotalNumBndryFaces), max = NumElems ...left elements */
    {
      ...
      for (int ValueIndex = 0; PreplotIsOk && ValueIndex < NumFaces; ValueIndex++)
        {
          ...
          if (PreplotIsOk)
            {
              LeftElems[ValueIndex] = (LgIndex)CurTValue;
              PreplotIsOk = WriteBinaryInt(BlckFile, (LgIndex)CurTValue-1); /* ...zero based */
            }
          GetToken(FALSE,TRUE,InFile);
        }
      /* Get right elements and compare to left elements. */
      for (int ValueIndex = 0; PreplotIsOk && ValueIndex < NumFaces; ValueIndex++)
        {
          if (CurToken != ValueToken)
            {
          ...
          if (PreplotIsOk)
            {
              if (CurTValue == LeftElems[ValueIndex])  
                ...
                PreplotIsOk = PreplotIsOk && WriteBinaryInt(BlckFile, (LgIndex)CurTValue-1); /* ...zero based */
            }
          if (PreplotIsOk)
            GetToken(FALSE,TRUE,InFile);
        }
      if (LeftElems) FREE_ARRAY(LeftElems,"LeftElems");
    }
  if (PreplotIsOk && (*TotalNumBndryFaces > 0))
    {
      ...
      PreplotIsOk = PreplotIsOk &&
                    (WriteBinaryInt(BlckFile, 0) &&
                     WriteBinaryInt(BlckFile, 0));
      for (DataIndex = 0; PreplotIsOk && DataIndex < *TotalNumBndryFaces; DataIndex++)
        {
          if (CurToken != ValueToken || CurTValue < 0)
            {
              ErrorMessage("Invalid value: expected a connection count of zero or greater, one for each boundary face.", TRUE);
              PreplotIsOk = FALSE;
            }
          FNOffset += (LgIndex)CurTValue;
          if (PreplotIsOk && FNOffset > *TotalNumBndryConns)
            {
              ErrorMessage("Sum of the counts specified in the boundary connection count section exceeds 'TotalNumBoundaryConnections.", TRUE);
              PreplotIsOk = FALSE;
            }
          if (PreplotIsOk)
            {
              PreplotIsOk = WriteBinaryInt(BlckFile, FNOffset);
              GetToken(FALSE,TRUE,InFile);
            }
        }
      if (PreplotIsOk)
          ...
          for (DataIndex = 0; PreplotIsOk && DataIndex < *TotalNumBndryConns; DataIndex++)
            {
              if (CurToken != ValueToken)
                {
                  ErrorMessage("Invalid token found: expected a boundary connection element.", TRUE);
                  PreplotIsOk = FALSE;
                }
              else if (CurTValue < 0)
                {
                  ErrorMessage("Boundary elements must be >= 0.", TRUE);
                  PreplotIsOk = FALSE;
                }
          ...
          for (DataIndex = 0; PreplotIsOk && DataIndex < *TotalNumBndryConns; DataIndex++)
            {
              if (CurToken != ValueToken)
                {
                  ErrorMessage("Invalid token found: expected a boundary connection zone.", TRUE);
                  PreplotIsOk = FALSE;
                }
              else if (CurTValue < 0)
                {
                  ErrorMessage("Boundary zones must be >= 0.", TRUE);
                  PreplotIsOk = FALSE;
                }
              ...
              if (ZoneBndry > 0)
                {
                  if (MaxPolyBndryConnElem[ZoneBndry-1] < PolyBndryConnElems[DataIndex])
                    {
                      MaxPolyBndryConnElem[ZoneBndry-1] = PolyBndryConnElems[DataIndex];
                      ZoneWithMaxConnElem[ZoneBndry-1] = CurZone + 1; /* Make 1-based for error message. */
                    }
                }
              else if (ZoneBndry == 0)
                {
                  if (MaxPolyBndryConnElem[CurZone] < PolyBndryConnElems[DataIndex])
                    {
                      MaxPolyBndryConnElem[CurZone] = PolyBndryConnElems[DataIndex];
...
static Boolean_t CheckPolyBndryConnections(int NumZones)
{
  Boolean_t IsOk = TRUE;
  if (MaxPolyBndryConnZone > NumZones)
    {
      char ErrMsg[256];
      sprintf(ErrMsg, "Invalid number \"%d\" for boundary connection zone in zone %d definition. "
                      "\nZone number must be from 1 to %d.", MaxPolyBndryConnZone, ZoneWithMaxBndryConnZone, NumZones);
      ErrorMessage(ErrMsg, FALSE);
      IsOk = FALSE;
    }
  Boolean_t IsValidElem = TRUE;
      ...
      for (I = 0; I < MaxNumZones; I++)
        {
          ...
          MaxPolyBndryConnElem[I] = 0;
          ZoneWithMaxConnElem[I] = 0;
        }
  ...
  if (PreplotIsOk)
    {
      ...
      if (InputIsPlot3D)
        ...
        {
          EndOfHeader = FALSE;
          FType = FULLFILE;   /* initialize FType so that if the file doesn't have FILETYPE, it is identified as FULLFILE */
          do
            {
              ...
              else if (CurToken == FILETYPEToken)
                {
                  /* read filetype */
                  FType = GetFileType();
                  GetToken(FALSE,TRUE,InFile);
...
/*Write out header information*/
          ...
          WriteI(PlotFile,1);
          /* Write out filetype */
          WriteI(PlotFile,(LgIndex)FType);
          DumpString(HeaderString);

          /* initialize zone number */
          CurZone = 0;

          if (!VariablesIdentified && (FType == GRIDFILE))
            {
              NoGridFileVars = TRUE;
              NumVars = 0;
            }

          /* Allow no variables in grid file but it must at least have connectivity */
          if (!VariablesIdentified && (FType != GRIDFILE))
            {
              if (CurToken == ZONEToken)
                {
                      ...
                      if (PreplotIsOk)
                        {
                          ...
                          if (ZoneIsFEPoly)
                            {
                              /* Enforce any rules that apply to an fepoly zone */
                              if (DataPacking[CurZone] == POINTPACKING)
                          ...
                          if (PreplotIsOk && NoGridFileVars)
                            {
                              ...
                              if (ZoneType[CurZone] == ORDERED)
                                {
                                  char TString[256];
                                  sprintf(TString,"The grid file %s contains no variables so all zones must be unstructured.",FinalInFileName);
                          ...
                          if (PreplotIsOk &&
                               ...
                               KMax[CurZone] != 0))
                            {
                              ...
                              if ((DataPacking[CurZone] == BLOCKPACKING) && !NoGridFileVars)
                                {
#if !defined TECPLOTKERNEL
                                  if (!Echo)
                                    printf("Var  : ");
                              ...
                              if (ZoneIsEnabled && PreplotIsOk && !NoGridFileVars)
                                {
                                  /* go back and re-write the var min/max values */
                              ...
                              /* Solution files MUST NOT contain connectivity or face neighbor info */
                              if (FType == SOLUTIONFILE)
                                {
                                  if (CurToken != ZONEToken           &&
                                      CurToken != GEOMETRYToken       &&
                                      CurToken != TEXTToken           &&
                                    ...
                                    {
                                      char TString[256];
                                      sprintf(TString,"Connectivity and/or face neighbor information exists in the solution file\n%s.",FinalInFileName);
                                      ErrorMessage(TString,TRUE);
                                    }
                                }
                              ...
                              /* Handle polygonal and polyhedral zone data */
                              if (ConnectivityShareZone[CurZone] == -1 &&
                                  FType != SOLUTIONFILE                &&
                                  ZoneIsFEPoly                         &&
                                  PreplotIsOk)
                                {       
                                  ...
                                  if (PreplotIsOk)
                                    {
                                      PreplotIsOk = ((CurToken == ValueToken ||
                                                      /* to accommodate the old ASCII format where the items are specified in the data section */
                                                      CurToken == IdentToken && TotalNumFaceNodes[CurZone] == -1) &&
                                                     ReadWritePolyMaps(JMax[CurZone],
                                                                       KMax[CurZone],
                                                                       IMax[CurZone],
                                                                       &TotalNumFaceNodes[CurZone],
                                                                       &TotalNumBndryFaces[CurZone],
                                                                       ...
                                                                       BlckFile));
                                      if (!PreplotIsOk)
                                        ErrorMessage("Error reading/writing face map data.", TRUE);
                                    }
                                }
                            }
                          ...
                          if (PreplotIsOk && ZoneIsEnabled)
                            {
                              ...
                              if (ZoneIsFEPoly)
                                {
                                  WriteI(PlotFile,KMax[CurZone]); /* NumFaces ...as of 110 */
                                  WriteI(PlotFile,TotalNumFaceNodes[CurZone]);  /* ...as of 111 */
                                  /* note we add 1 for no neighboring element as a convenience for the user */
                                  if (TotalNumBndryFaces[CurZone] > 0)
                                    WriteI(PlotFile, TotalNumBndryFaces[CurZone]+1); /* ...as of 111 */
                                  else
                                    WriteI(PlotFile,TotalNumBndryFaces[CurZone]); /* ...as of 111 */
                                  WriteI(PlotFile,TotalNumBndryConns[CurZone]); /* ...as of 111 */
                                }
 ...
 */
      ...
      if (PreplotIsOk)
        PreplotIsOk = CheckPolyBndryConnections(CurZone);
    }
