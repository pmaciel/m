...
Register< mtransform,t_zedge > mt_zedge("-tzedge","[str:...] zone edge extraction");
...
void t_zedge::transform(GetPot& o, mmesh& m)
{
  ...
  // (has to be done by index otherwise modifying the zones vector can
  // invalidate references & iterators)
  vector< unsigned > zm;
  ...
  mzone& z = m.vz[ zm[0] ];  // this is the zone that edges the others
