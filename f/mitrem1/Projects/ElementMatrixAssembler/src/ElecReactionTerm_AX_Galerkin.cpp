...
//---------------------------------------------------------------------------
ElecReactionTerm_AX_Galerkin::ElecReactionTerm_AX_Galerkin(unsigned nDimensions_, unsigned nNodes_, unsigned nVariables_, MITReM* mitrem_, BoundaryElementProps* boundaryElementProps_)
  : ElecReactionTerm(nDimensions_, nNodes_,nVariables_, mitrem_, boundaryElementProps_)
{
}
//---------------------------------------------------------------------------
ElecReactionTerm_AX_Galerkin::~ElecReactionTerm_AX_Galerkin()
{
}
//---------------------------------------------------------------------------
void ElecReactionTerm_AX_Galerkin::calcVec(double* boundaryElementVec, DoubleVectorList coordinates, DoubleVectorList concentrations, DoubleList potentials, DoubleList temperatures, DoubleList densities, DoubleList surfaceGasFractions, unsigned nElecReactions, IndexList elecReactions, double electrodePotential, unsigned nGasReactions, IndexList gasReactions)
{
    ...
    for (unsigned m=0; m<nNodes; m++)
    {
      ...
      for (unsigned k=0; k<nAgentsRed; k++)
      {
        ...
        for (unsigned l=0; l<nAgentsRed; l++)
        {
          ...
          boundaryElementJac[eqmi][var(m,j)] += R[m][m]*DvDCRed[m][r][l]*elementSize12*stoichRed;
          boundaryElementJac[eqmi][var(p,j)] += R[m][p]*DvDCRed[p][r][l]*elementSize12*stoichRed;
        }
        ...
        for (unsigned l=0; l<nAgentsOxi; l++)
        {
          ...
          boundaryElementJac[eqmi][var(m,j)] += R[m][m]*DvDCOxi[m][r][l]*elementSize12*stoichRed;
          boundaryElementJac[eqmi][var(p,j)] += R[m][p]*DvDCOxi[p][r][l]*elementSize12*stoichRed;
        }

        // derivative of v(s) by U
        boundaryElementJac[eqmi][var(m,nIons)] += R[m][m]*DvDU[m][r]*elementSize12*stoichRed;
        boundaryElementJac[eqmi][var(p,nIons)] += R[m][p]*DvDU[p][r]*elementSize12*stoichRed;
      }
      ...
      for (unsigned k=0; k<nAgentsOxi; k++)
      {
        ...
        {
          unsigned j = mitrem->getElecReactionAgentsRed(s,l);
          boundaryElementJac[eqmi][var(m,j)] += R[m][m]*DvDCRed[m][r][l]*elementSize12*stoichOxi;
          boundaryElementJac[eqmi][var(p,j)] += R[m][p]*DvDCRed[p][r][l]*elementSize12*stoichOxi;
        }
        ...
        {
          unsigned j = mitrem->getElecReactionAgentsOxi(s,l);
          boundaryElementJac[eqmi][var(m,j)] += R[m][m]*DvDCOxi[m][r][l]*elementSize12*stoichOxi;
          boundaryElementJac[eqmi][var(p,j)] += R[m][p]*DvDCOxi[p][r][l]*elementSize12*stoichOxi;
        }

        // derivative of v(s) by U
        boundaryElementJac[eqmi][var(m,nIons)] += R[m][m]*DvDU[m][r]*elementSize12*stoichOxi;
        boundaryElementJac[eqmi][var(p,nIons)] += R[m][p]*DvDU[p][r]*elementSize12*stoichOxi;
      }
    }
  }
}
