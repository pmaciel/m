...
void IJacobian_turb_uncoupled()
{
  ...
  for (int ic=0; ic<Ncell; ++ic) {
    ...
    for (int inc=0; inc<Nvtcell; ++inc)
      for (int jnc=0; jnc<Nvtcell; ++jnc)
        ls_aztec_turb1->A(No_local[inc].node,No_local[jnc].node) += No_local[inc].C[jnc];

    scacde(ic,iv_turb2,No_local,vol,inc_min,scaconv,nulam+nuturb/Sig2,0.,0.,1);
    for (int inc=0; inc<Nvtcell; ++inc)
      for (int jnc=0; jnc<Nvtcell; ++jnc)
        ls_aztec_turb2->A(No_local[inc].node,No_local[jnc].node) += No_local[inc].C[jnc];

    /* add residuals to global arrays */
    for (int inc=0; inc<Nvtcell; ++inc) {
      ls_aztec_turb1->B(e2n[ic].n[inc]) += No_local[inc].Res[iv_turb1];
      ls_aztec_turb2->B(e2n[ic].n[inc]) += No_local[inc].Res[iv_turb2];
    }
  ...
  turb_source_node();
  turb_wallbc(0,0,ls_aztec_turb1,ls_aztec_turb2);
  ...
  for (int ig=1; ig<=Nbcgroup; ++ig) {
    if (BCgroup[ig].type==IBFIXV || BCgroup[ig].type==IBPERE)
      for (int inb=1; inb<=BCgroup[ig].nnode; ++inb) {
        ...
        ls_aztec_turb1->zerorow(inu); ls_aztec_turb1->A(inu,inu) = 1.;
        ls_aztec_turb2->zerorow(inu); ls_aztec_turb2->A(inu,inu) = 1.;
      }
  }
